...
## Slice 7：导入仅外部存在的 custom_phrase
...
- [x] 一键导入差异预览中的「仅外部存在」
- [x] 已存在相同词语+编码则跳过，不覆盖
- [x] 导入后刷新差异；不改 custom_phrase.txt
...
- [x] 主操作收到导航栏，减少列表里的工具按钮
- [x] 剪贴板 / 常用语 / 词库：点按复制，滑动编辑或删除
- [x] 去掉行内常驻「复制」按钮
- [x] 设置页去掉未完成开关和「MVP 骨架」文案

### 热修复（v0.2.4）

- [x] 剪贴板采集按钮不再被列表 loading 禁用，无新内容会提示
- [x] 文本 Tab 图标改为 `text.alignleft`，避免被读成「格式」
- [x] 差异预览打开时隐藏「连接目录」，避免关闭按钮误触选目录

### 热修复（v0.2.5）

- [x] 文本页改为多行 TextField，不再使用 Tab 内空白的 Editor
- [x] 差异预览打开时移除词库整个 toolbar，避免关闭点穿到新建
- [x] 预览关闭按钮改为「完成」

### 热修复（v0.2.6）

- [x] 差异预览改为独立 `Navigation.present`，不再用底层 List sheet
- [x] 新建词条移入列表，导航栏不再与完成按钮重叠

### 热修复（v0.2.7）

- [x] 文本工具改为系统列表行，不再用挤扁的圆按钮
...
- [x] 设置改为分组目录：使用习惯、工作区、词库安全、关于
- [x] 色块 SF Symbol 仅用于设置入口，不改常用语/词库列表
- [x] 启动打开常用语或词库，下次打开生效
- [x] 复制后提示可开关；点按复制成功显示「已复制」
- [x] 工作区授权管理仍可在设置里断开/重连，预览差异仍在词库页
...
- [x] 词库工作区行改为色块 SF Symbol + 标题/说明 + chevron
- [x] 去掉容器沙盒路径
- [x] 点按仍预览差异，右滑仍断开
