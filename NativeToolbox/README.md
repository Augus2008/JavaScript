...
- 万象 `custom_phrase.txt` 只读解析与差异预览，不写入外部文件。

- 万象 `custom_phrase.txt` 安全提交：哈希冲突检测、备份、临时写入与校验。

- 可将万象「仅外部存在」的 custom_phrase 导入内部词库，相同词语+编码不覆盖。

- 设置页改为分组入口：使用习惯、工作区、词库安全、关于；可改启动页和复制提示。

- 词库工作区行改为色块图标入口，去掉沙盒路径。

仍不修改 userdb / gram / 官方 dicts。
